...
| Adapt `RecordingManager` from `@repo/tantrica-core` | Strategy pattern: inject `VSCodeEventCapture` |

### 2.3 Export & Upload [~2 days]

| Task                   | Description                                                                  |
| ---------------------- | ---------------------------------------------------------------------------- |
| `TantricaFileWriter`   | Serialize recording to .tantrica format (using `@repo/tantrica-core/format`) |
...
### 3.1 Fork Mechanism [~3 days]

| Task                                  | Description                                       |
| ------------------------------------- | ------------------------------------------------- |
| "Fork" button in PlaybackViewer       | Appears during playback, snapshots state          |
| Fork creates editable Monaco instance | Clones current content + cursor position          |
| Forked session management             | Track forked state separately from playback state |
| Return to playback                    | Button to return from fork view back to playback  |
...
| Phase 2   | ~10 days     | VS Code extension for recording + export + upload         |
| Phase 3   | ~7 days      | Interactive playback with fork and annotations            |
| **Total** | **~28 days** | Full product                                              |
...
Phase 2.1 (extension scaffold) → after 1.3 (needs tantrica-core)
Phase 2.2 (event capture)      → after 2.1
Phase 2.3 (export/upload)      → after 2.2
Phase 2.4 (sidebar UI)         → after 2.3

Phase 3.1 (fork)               → after 1.4
Phase 3.2 (annotations)        → after 1.4
Phase 3.3 (progressive load)   → after 1.2, 1.4
```

Phase 1.1 and 1.2 can be done in parallel. Phase 2 can start as soon as `tantrica-core` package exists (1.3).
